...
const defaultTheme = 'https://image-cdn-ak.spotifycdn.com/image/ab6761860000eab1144139d91f0c7d4432ec9ef6'
const defaultvideo = 'https://www.youtube.com/embed/WvqKGMBTI8Y?autoplay=1&mute=1&controls=0&loop=1&playlist=WvqKGMBTI8Y'
const HomeCom = ({fet, gp, ImgThumb, stream, kamin, setSec, width}) => {
 ...
 const ran = getRandomIntExcept(3, 5, 0);
 fetch(fet + '/bnk48/getmemberby?filter=gen&param=' + ran + '&tstamp=' + Math.floor( new Date().getTime()  / 1000), {
  ...
  <hr />
  {onMonth ? (
    <CardHeader className='mb-5' title={(<h3 data-aos="flip-up">BNK48 Members Birthday on this month</h3>)} subheader={moment().format('MMMM YYYY')} />
  ) : (
    <CardHeader className='mb-5' title={(<h3 data-aos="flip-up">BNK48 Members Birthday in today</h3>)} subheader={moment().format('DD MMMM YYYY')} />
  )}
  {Loaded1 ? (
      ...
      {birth.length > 0 ? birth.map((item, i) => (
           ...
           <CardMedia
               ...
               <CardContent>
                   <h5>{item.name}</h5>
                   <p>Birthday: {moment(item.birthday, 'YYYY-M-DD').format('DD MMMM YYYY')} ({moment(item.birthday, 'YYYY-M-DD').format('M') == new Date().getMonth() + 1 && parseInt(moment(item.birthday, 'YYYY-M-DD').format('D')) - new Date().getDate() > 0 ? (new Date().getFullYear() - new Date(item.birthday).getFullYear()) + ' years old | ' + ((parseInt(moment(item.birthday, 'YYYY-M-DD').format('D')) - new Date().getDate()) == 1 ? 'in tomorrow' : (parseInt(moment(item.birthday, 'YYYY-M-DD').format('D')) - new Date().getDate()) + ' day(s) to go') : moment(item.birthday, 'YYYY-M-DD').format('DD MMMM') == moment().format('DD MMMM') ? 'Happy ' + (new Date().getFullYear() - new Date(item.birthday, 'YYYY-M-DD').getFullYear()) + ' years old!' : (new Date().getFullYear() - new Date(item.birthday, 'YYYY-M-DD').getFullYear()) + ' years old | already passed'})</p>
               </CardContent>
             </CardActionArea>
              </Card> 
           </div>
