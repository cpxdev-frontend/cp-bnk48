    ...
    return ( 
        ...
        {window.innerWidth > 1100 && (
          ...
          {localStorage.getItem('lowgraphic') == null ? (
            ...
            <iframe src="https://www.youtube.com/embed/wO7kV905DnA?autoplay=1&mute=1&controls=0&loop=1&playlist=wO7kV905DnA" frameborder="0"></iframe>
          </div>
          ) : (
  ...
  {Loaded1 ? (
      ...
      {birth.length > 0 ? birth.map((item, i) => (
           ...
           <CardMedia
               ...
               <CardContent>
                   <h5>{item.name}</h5>
                   <p>Birthday: {moment(item.birthday).format('ddd DD MMMM YYYY')} ({moment(item.birthday).format('M') == new Date().getMonth() + 1 && parseInt(moment(item.birthday).format('D')) - new Date().getDate() > 0 ? (parseInt(moment(item.birthday).format('D')) - new Date().getDate()) + ' day(s) to go' : (new Date().getFullYear() - new Date(item.birthday).getFullYear()) + ' years old'})</p>
               </CardContent>
             </CardActionArea>
              </Card> 
           </div>
