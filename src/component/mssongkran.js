...
const fetchapi = () => {
    ...
    fetch(fet + '/bnk48/tkxlist/0xE4629861b811d23Afde5E4476DFA79003C0E83d1', {
        ...
        fetch(fet + '/bnk48/tkxlist/0xa8E49Dce482E68df7D89509b4810dc466bd6f24A?chartLimit=6', {
          method :'post'
      })
          .then(response => response.json())
          .then(datax => {
...
<TableContainer>
                ...
                <caption className='text-right'>Note: These result is syncing from <a href='https://scan.tokenx.finance/address/0xE4629861b811d23Afde5E4476DFA79003C0E83d1/read-contract' target='_blank'>Token X Smart Contract (TKX Scan)</a> and every value may change. Please check the official results at BNK48 Official soon.</caption>
                  ...
                  {data != null && bnk.length > 0 && cgm.length > 0 ? data.list.map((item, i) => (
                     <TableBody key={item.header + item.desc} onClick={() => item.desc.includes('BNK48') ? History.push('/member/' + item.header.toLowerCase()) : item.desc.includes('CGM48') ? window.open('//cp-cgm48.pages.dev/member/' + item.header.toLowerCase(), '_target') : ''}
