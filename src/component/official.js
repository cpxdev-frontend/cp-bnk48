    ...
    return ( 
        ...
        {Loaded ? (
             ...
             {Arr.length > 0 ? Arr.map((item,i) => (
                ...
                <CardContent>
                    <Typography variant="h6" dangerouslySetInnerHTML={width < 700 ? { __html: removeurl(item.title.replace(new RegExp('\n', 'g'), '<br />')) } : { __html: removeurl(item.title) }}>
                    </Typography>
                    <hr />
